...
static void pkgi_do_main(pkgi_input* input)
{
    ...
    if (input)
    {
        if (input->active & pkgi_cancel_button())
        {
            ...
            pkgi_dialog_ok_cancel("\xE2\x98\x85  Game Slayer" PKGI_VERSION "  \xE2\x98\x85", _("Exit to XMB?"), &cb_dialog_exit);
        }

        if (input->active & PKGI_BUTTON_SELECT)
        {
            input->pressed &= ~PKGI_BUTTON_SELECT;
            pkgi_dialog_message("\xE2\x98\x85  Game Slayer" PKGI_VERSION "  \xE2\x98\x85",
                                "             By Nathan-Busse\n\n");
        }
...
{
    char title[256];
    pkgi_snprintf(title, sizeof(title), "Game Slayer v%s - %s", PKGI_VERSION, content_type_str(config.content));
...
static void pkgi_update_check_thread(void)
{
    ...
    LOG("checking latest Game Slayer version at %s", PKGI_UPDATE_URL);
    ...
    if ((value = json_parse(jobj, "name")) == NULL || !pkgi_memequ("Game Slayer ", value, 8) || pkgi_stricmp(PKGI_VERSION, value+10) == 0)
    {
    ...
    {
        pkgi_dialog_message(update_item.name, _("Successfully downloaded Game Slayer update"));
